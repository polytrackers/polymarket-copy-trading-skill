...
MCP catalog fingerprint: sha256-bcb9012ec434616a2f7e5d197a4bc6b92d10576926404ad3335855c6cd4793d2
...
- `pt_webhook_update` accepts any nonempty subset of `url`, `filters`, and `secret`. Supply only the fields to change; a filters-only or secret-only update leaves the URL untouched.
...
4. Check `lifecycle_warnings`, `lifecycle_contradiction`, and `outcome_side_mapping`; use an explicit resolved winner only to settle or close tracked positions, and do not treat past-end, closed, stale, contradictory live-order-book, or unmapped named-outcome markets as directly actionable.
